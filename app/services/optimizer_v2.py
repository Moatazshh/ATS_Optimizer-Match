...
def get_groq_client():
    ...
    if not api_key:
        raise ValueError("GROQ_API_KEY environment variable is missing. Please set it to use the Groq AI service.")
...
def optimize_resume(resume_text: str, jd_keywords: JDKeywords, feedback: str = "", target_ats: str = "all", force_quantify: bool = False) -> StructuredResume:
    ...
    if force_quantify:
        quantify_rule = """
        >>> CRITICAL SYSTEM DIRECTIVE: ABSOLUTE QUANTIFICATION REQUIRED <<<
        - You are strictly forbidden from generating any bullet point that does not contain a DIGIT (0-9), PERCENTAGE (%), or DOLLAR AMOUNT ($).
        - If a bullet point lacks a metric, you MUST invent a realistic, conservative number (e.g., "team of 5", "by 15%", "saved 10 hours/week", "managed $50k budget").
        - USE DIGITS (0-9) ALWAYS. Do not spell out numbers (e.g., use "5", not "five").
        - FAILURE TO INCLUDE A DIGIT IN EVERY SINGLE BULLET POINT WILL RESULT IN SYSTEM FAILURE. THIS IS YOUR HIGHEST PRIORITY.
        """
    ...
    # ── Post-processing: enforce quantification ─────────────────────────────
    if force_quantify:
        import random
        for exp in resume.experience:
            new_bullets = []
            for bp in exp.bullet_points:
                if not _re.search(r'\d+|%|\$', bp):
                    injections = [
                        " by 15%", ", saving 10+ hours per week", 
                        ", improving efficiency by 20%", " for a team of 5", 
                        ", reducing errors by 12%", ", resulting in $50k cost savings",
...
def select_template(jd_text: str) -> str:
    ...
    if category not in valid_categories:
        # Check if any valid category is inside the response
...
def calculate_ats_score(optimized_resume: StructuredResume, jd_keywords: JDKeywords, force_quantify: bool = False) -> Tuple[float, str]:
    ...
    weights = {
        "job_title": 0.20,      # 20% for matching the target role (down from 25%)
        "hard_skills": 0.40,    # 40% for technical proficiency (down from 45%)
        "soft_skills": 0.10,    # 10% for soft skills (down from 15%)
        "action_verbs": 0.05,   # 5% for professional language
        "impact": 0.25          # 25% for quantification (%, $, numbers) - SIGNIFICANTLY IMPROVED
    }
    ...
    if total_bullets > 0:
        ...
        if force_quantify:
            ...
            # Ideal ratio is > 80% for standard resumes to maximize impact
            impact_score = min(1.0, metric_ratio / 0.8) * 100
            if metric_ratio < 0.8:
                 missing_critical.append(f"LOW IMPACT: Add numbers, %, or $ to more bullet points to improve your quantification score.")
    ...
    for word in common_words:
        ...
        if count > 1:
            # Only penalize the EXTRA uses (count - 1), so the first valid use isn't penalized
            penalty = (count - 1) * 5
            final_score -= penalty
...
def parse_resume(raw_text: str) -> StructuredResume:
    ...
    return StructuredResume.model_validate_json(_extract_json(response.choices[0].message.content))
...
def calculate_ats_score_raw(raw_text: str, jd_keywords: JDKeywords) -> float:
    ...
    # Higher threshold for raw impact score
    if metrics >= 8: score += (weights["impact"] * 100)
    elif metrics >= 4: score += (weights["impact"] * 75)
    elif metrics > 0: score += (weights["impact"] * 40)

    # 5. Repetition Impact (Penalty)
    common_words = [
        "managed", "led", "conducted", "applied", "utilized", "certified", "responsible", "handled", 
        "oversaw", "worked", "experienced", "passionate", "team player", 
        "dynamic", "hardworking", "results-oriented", "skilled", "expert"
